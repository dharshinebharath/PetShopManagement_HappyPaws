...
import org.springframework.data.domain.Sort; // ✅ IMPORTANT IMPORT
...
public class SupplierService implements SupplierInterface {
    ...
    public ApiResponse<List<SupplierResponseDTO>> saveAll(List<SupplierRequestDTO> suppliers) {
        ...
        return new ApiResponse<>("Suppliers saved successfully", true, responseList);
    }

    // ✅ GET ALL (🔥 FIXED WITH SORT)
    @Override
    public ApiResponse<List<SupplierResponseDTO>> getAll() {

        List<SupplierResponseDTO> data =
                supplierRepository
                        .findAll(Sort.by(Sort.Direction.ASC, "supplierId")) // 🔥 FIX HERE
                        .stream()
                        .map(this::toDto)
                        .toList();

        return new ApiResponse<>("Fetched all suppliers", true, data);
    }
    ...
    public ApiResponse<SupplierResponseDTO> getSupplierById(long supplierId) {
        ...
        return new ApiResponse<>("Supplier fetched successfully", true, toDto(entity));
    }
    ...
    public ApiResponse<String> deleteSupplier(long supplierId) {
        ...
        return new ApiResponse<>("Deleted successfully", true,
                "Supplier deleted with id: " + supplierId);
    }
    ...
    public ApiResponse<SupplierResponseDTO> updateSupplier(Long id, SupplierRequestDTO dto) {
        ...
        return new ApiResponse<>("Updated successfully", true, toDto(updated));
    }
    ...
    public ApiResponse<List<SupplierResponseDTO>> getSuppliersByPet(String petName) {
        ...
        List<SupplierResponseDTO> data =
                supplierRepository.findSuppliersByPetName(petName)
                        .stream()
                        .map(this::toDto)
                        .toList();
