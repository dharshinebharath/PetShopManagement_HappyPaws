...
public class GroomingServicesService implements GroomingServicesInterface {
	...
	public GroomingServices getGroomingServiceById(long id) {
		return groomingServicesRepository.findById(id).orElseThrow(()->new ResponseStatusException(
                HttpStatus.NOT_FOUND, "Grooming Service Not Found")); 
	}
	
	@Override
	public void deleteGroomingServiceById(long id)
	{
		GroomingServices existing=groomingServicesRepository.findById(id).orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND, "Grooming Services Not Found"));
	...
	public GroomingServices updateGroomingService(long id, @Valid GroomingServices service) {
		GroomingServices existing = groomingServicesRepository.findById(id)
	            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Grooming Services Not Found"));
